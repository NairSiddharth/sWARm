...
def calculate_park_factors():
    ...
    for stadium in games_df['Stadium_Clean'].unique():
        ...
        team_home_games = stadium_games[stadium_games['home'] == home_team]
        team_away_games = games_df[games_df['away'] == home_team]

        if len(team_home_games) == 0 or len(team_away_games) == 0:
            continue

        # Calculate offensive stats (runs + total bases as proxy for offensive environment)
        home_offense = (team_home_games['home-score'].mean() +
                       team_home_games['Total Bases - Home'].mean() / 10.0)
        away_offense = (team_away_games['away-score'].mean() +
                       team_away_games['Total Bases - Away'].mean() / 10.0)

        # Calculate park factor: (Home Performance / Away Performance) * 100
        if away_offense > 0:
            park_factor = (home_offense / away_offense) * 100

            # Apply stronger park factor effects - amplify deviations from 100
            if park_factor > 100:
                # Hitter-friendly parks - amplify the effect
                amplified_factor = 100 + (park_factor - 100) * 1.5
            else:
                # Pitcher-friendly parks - amplify the effect
                amplified_factor = 100 - (100 - park_factor) * 1.5

            park_stats[stadium] = round(amplified_factor, 1)

    # Display results
    print(f"\nCalculated enhanced park factors for {len(park_stats)} stadiums:")
    extreme_parks = []
    for stadium, pf in sorted(park_stats.items(), key=lambda x: x[1], reverse=True):
        park_type = "HITTER" if pf > 105 else "PITCHER" if pf < 95 else "NEUTRAL"
        if pf > 110 or pf < 90:
            extreme_parks.append((stadium, pf, park_type))
        if len(extreme_parks) < 10:  # Show first 10 extreme parks
            print(f"  {stadium:<30} {pf:>5.1f} ({park_type})")
...
def apply_enhanced_pitcher_park_adjustments(pitcher_stats, player_name, team, park_factors=None):
    ...
    # Apply boost to key performance metrics with stronger effects
    if 'ERA' in adjusted_stats:
        # Lower ERA in hitter-friendly parks is MORE impressive
        adjusted_stats['ERA_park_adj'] = adjusted_stats['ERA'] / pitcher_boost

    if 'WHIP' in adjusted_stats:
        adjusted_stats['WHIP_park_adj'] = adjusted_stats['WHIP'] / pitcher_boost
