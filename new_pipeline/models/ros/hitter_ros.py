"""
Complete ROS Ensemble for Hitters
...
- DirectROSForecaster: ALL 56 ROS features as exogenous variables + automatic lags from WAR
...
# ROS Hitter Features (65 total)
ROS_HITTER_FEATURES = [
    # ===== Current Performance (from HITTER_MODEL_FEATURES) =====
    'K%', 'BB%', 'AVG', 'OBP', 'SLG', 'GDP',
    'Positional_WAR', 'Enhanced_Baserunning', 'Enhanced_Defense',

    # ===== Current Season Model Output =====
    'current_predicted_war_rate',  # Output from HitterEnsemble

    # ===== Baseline Comparisons (wOPS and GDP) =====
    # wOPS = (1.8 × OBP + SLG) / 2.8 (weighted OPS)
    'career_wOPS', 'healthy_wOPS', 'peak_wOPS', 'recent_3yr_wOPS',
    'wOPS_vs_healthy', 'wOPS_vs_recent', 'wOPS_vs_peak', 'maintenance_ratio_wOPS',
    # GDP (Ground into Double Play)
    'career_GDP', 'healthy_GDP', 'peak_GDP', 'recent_3yr_GDP',
    'GDP_vs_healthy', 'GDP_vs_recent', 'GDP_vs_peak', 'maintenance_ratio_GDP',
    ...
    # ===== Baserunning Projection (Sprint Speed Age Curves) =====
    'projected_baserunning_ros',  # Age-adjusted 3yr average
    'baserunning_age_factor',  # Sprint speed age adjustment (0.7-1.0)
    'baserunning_3yr_avg',  # Raw 3-year average (before age adjustment)
    'baserunning_trend',  # Recent trend (positive = improving)

    # ===== Defense Projection (Sample-Size Weighted, No Age Curves) =====
    'projected_defense_ros',     # Innings-weighted projection
    'defense_sample_weight',     # Reliability (0-1)
    'defense_3yr_avg',           # Unadjusted baseline
    'defense_current_rate',      # Annualized rate per 1200 inn
    'defense_trend',             # Recent 3yr trend
    ...
    # ===== Usage Context =====
    'PA',  # Plate appearances to date
    'Games',  # Games played to date
    'season_completion_pct'  # Percent of season completed (0.0-1.0)
]


class HitterROSEnsemble(BaseEnsemble):
    """
    Complete ROS prediction ensemble for hitters.
    ...
    def __init__(
        ...
        Args:
            weights: Weights for [Direct, Temporal, Baseline]
                Default: [0.5, 0.4, 0.1]
            feature_columns: List of feature columns (default: ROS_HITTER_FEATURES)
            target_column: Target column name (default: 'WAR_per_600')
        """
        if weights is None:
            weights = [0.5, 0.4, 0.1]

        super().__init__(player_type='hitter', weights=weights)

        self.feature_columns = feature_columns if feature_columns is not None else ROS_HITTER_FEATURES
        self.target_column = target_column

        # Initialize component models
        self.direct_model = DirectROSForecaster(player_type='hitter', lags=2)
        self.temporal_model = DartsTemporalEnsemble(player_type='hitter')
        self.baseline_model = MultiQuantileHistGB(player_type='hitter')
    ...
    def fit(
        ...
        Args:
            historical_df: Historical data with columns:
                ...
                - Year: Year
                - All feature columns
                - Target column (WAR_per_600 or WAR_per_162)
        ...
        print(f"Fitting HitterROSEnsemble on {len(historical_df)} samples...")
    ...
    def predict(
        ...
        Example:
            >>> # Predict 2025 ROS for qualified hitters (mixed history lengths)
            ...
            >>> ros_predictions
            array([4.2, 3.8, 5.1, ...])  # Secondhalf WAR predictions
        """
    ...
    def predict_with_elite_adjustments(
        ...
        Args:
            current_df: Current season data (must include 'WAR', 'PA', 'Games', 'playerid')
        ...
        for idx, row in current_df.iterrows():
            ...
            if is_candidate:
                ...
                remaining_usage = calculate_remaining_usage(
                    row, 'hitter', team_games_dict, league_median_games
                )
